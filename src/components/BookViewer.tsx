import { libraryFacts } from '../game/data';
...
export default function BookViewer({ bookIndex, onClose }: Props) {
  const book = libraryFacts[bookIndex];
  if (!book) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center pixel-overlay" onClick={onClose}>
      <div className="pixel-card w-[500px] max-w-[90vw]" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-foreground text-sm">{book.title}</h2>
